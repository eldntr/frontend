...
{{-- Content --}}
<div class= "max-w-screen-xl px-4 mx-auto 2xl:px-0 py-4 grid lg:grid-cols-3 md:grid-cols-2 gap-4 items-center justify-center scale-90">
    @foreach ( $products as $item )
        <a href="#" class="group relative block overflow-hidden">
            ...
            <div class="relative border border-gray-100 bg-white p-6">
                <span class="whitespace-nowrap rounded bg-teal-700 text-white px-3 py-1.5 text-xs font-medium"> {{ $item->category()->name }} </span>
            
                <h3 class="mt-4 text-lg font-medium text-gray-900">{{ $item->name }}</h3>
            
                <p class="mt-1.5 text-sm text-gray-700">Rp. {{ $item->price }}</p>
